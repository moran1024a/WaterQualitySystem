...
static int wq_cmp_record(const void *a, const void *b)
{
    ...
    if (mb)
        return -1;
    if (ra->value[p] < rb->value[p])
        return (g_sort_order == WQ_SORT_ASC) ? -1 : 1;
    if (ra->value[p] > rb->value[p])
        return (g_sort_order == WQ_SORT_ASC) ? 1 : -1;
    return 0;
}
...
void wq_print_record(const WaterQualityRecord *record)
{
    ...
    {
        if (record->missing[i])
            printf("| %s=NaN ", wq_parameter_to_string((WQParameter)i));
        else
            printf("| %s=%.3f ", wq_parameter_to_string((WQParameter)i), record->value[i]);
    }
    printf("\n");
}
...
void wq_print_page(const WaterQualityDataset *dataset, size_t page_index, size_t page_size)
{
    ...
    for (i = start; i < end; ++i)
        wq_print_record(&dataset->records[i]);
}
...
{
    size_t i;
    if (src == NULL || dst == NULL || parameter < 0 || parameter >= WQ_PARAM_COUNT || min_value > max_value)
        return WQ_ERROR;
    ...
    for (i = 0U; i < src->size; ++i)
    {
        ...
        {
            if (wq_dataset_push(dst, r) != WQ_SUCCESS)
                return WQ_ERROR;
        }
    }
    return WQ_SUCCESS;
...
int wq_delete_record(WaterQualityDataset *dataset, size_t record_index)
{
    ...
    if (dataset == NULL || record_index >= dataset->size)
        return WQ_ERROR;
    for (i = record_index + 1U; i < dataset->size; ++i)
        dataset->records[i - 1U] = dataset->records[i];
    dataset->size--;
    return WQ_SUCCESS;
}
                                  ...
                                  double max_value)
{
    ...
    if (dataset == NULL || parameter < 0 || parameter >= WQ_PARAM_COUNT || min_value > max_value)
        return 0U;
