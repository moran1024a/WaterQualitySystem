...
int wq_dataset_reserve(WaterQualityDataset *dataset, size_t new_capacity)
{
    ...
    if (dataset == NULL) return WQ_ERROR;
    if (new_capacity <= dataset->capacity) return WQ_SUCCESS;
    if (new_capacity > (SIZE_MAX / sizeof(WaterQualityRecord))) return WQ_ERROR;

    new_records = (WaterQualityRecord *)realloc(dataset->records, new_capacity * sizeof(WaterQualityRecord));
    if (new_records == NULL) return WQ_ERROR;
...
{
    size_t new_capacity;
    if (dataset == NULL || record == NULL) return WQ_ERROR;

    if (dataset->size >= dataset->capacity) {
        if (dataset->capacity == 0U) new_capacity = WQ_INITIAL_CAPACITY;
        else {
            if (dataset->capacity > SIZE_MAX / 2U) return WQ_ERROR;
            new_capacity = dataset->capacity * 2U;
        }
        if (wq_dataset_reserve(dataset, new_capacity) != WQ_SUCCESS) return WQ_ERROR;
    }
...
WaterQualityDataset *wq_read_csv(const char *filename, DataOverview *overview)
{
    ...
    if (overview != NULL) memset(overview, 0, sizeof(*overview));
    ...
    while (fgets(line, sizeof(line), fp) != NULL) {
        ...
        for (col = 0U; col < (size_t)WQ_PARAM_COUNT; ++col) {
            ...
            if (comma != NULL) {
                ...
                if (n >= sizeof(field)) n = sizeof(field) - 1U;
            ...
            if (wq_is_missing_text(field)) {
                ...
                if (overview != NULL) overview->missing_values++;
            } else {
                double value;
...
int wq_write_csv(const char *filename, const WaterQualityDataset *dataset)
{
    FILE *fp;
    size_t i, j;
    if (filename == NULL || dataset == NULL) return WQ_ERROR;
    ...
    for (i = 0U; i < dataset->size; ++i) {
        for (j = 0U; j < (size_t)WQ_PARAM_COUNT; ++j) {
            ...
            if (rc < 0) { fclose(fp); return WQ_ERROR; }
            if (j + 1U < (size_t)WQ_PARAM_COUNT && fputc(',', fp) == EOF) { fclose(fp); return WQ_ERROR; }
        }
        if (fputc('\n', fp) == EOF) { fclose(fp); return WQ_ERROR; }
    }

    if (fclose(fp) != 0) return WQ_ERROR;
    return WQ_SUCCESS;
}
...
WaterQualityDataset *wq_read_binary(const char *filename)
{
    ...
    if (fread(&header, sizeof(header), 1U, fp) != 1U) { fclose(fp); return NULL; }
    ...
    if (dataset == NULL) { fclose(fp); return NULL; }
...
int wq_write_binary(const char *filename, const WaterQualityDataset *dataset)
{
    ...
    if (fwrite(&header, sizeof(header), 1U, fp) != 1U) { fclose(fp); return WQ_ERROR; }
...
int wq_read_binary_record(const char *filename, size_t index, WaterQualityRecord *record_out)
{
    ...
    if (filename == NULL || record_out == NULL) return WQ_ERROR;
    fp = fopen(filename, "rb");
    if (fp == NULL) return WQ_ERROR;

    if (fread(&header, sizeof(header), 1U, fp) != 1U) { fclose(fp); return WQ_ERROR; }
    ...
    if (fseek(fp, offset, SEEK_SET) != 0) { fclose(fp); return WQ_ERROR; }
    if (fread(record_out, sizeof(WaterQualityRecord), 1U, fp) != 1U) { fclose(fp); return WQ_ERROR; }
                               ...
                               StorageBenchmark *binary_result)
{
    ...
    begin = clock();
    if (wq_write_csv(csv_file, dataset) != WQ_SUCCESS) return WQ_ERROR;
    end = clock();
    csv_result->write_seconds = ((double)(end - begin) ) / CLOCKS_PER_SEC;

    begin = clock();
    { WaterQualityDataset *tmp = wq_read_csv(csv_file, NULL); if (tmp == NULL) return WQ_ERROR; wq_dataset_destroy(tmp); }
    end = clock();
    csv_result->read_seconds = ((double)(end - begin) ) / CLOCKS_PER_SEC;

    fp = fopen(csv_file, "rb");
    if (fp == NULL) return WQ_ERROR;
    if (fseek(fp, 0L, SEEK_END) != 0) { fclose(fp); return WQ_ERROR; }
    ...
    begin = clock();
    { WaterQualityDataset *tmp = wq_read_binary(binary_file); if (tmp == NULL) return WQ_ERROR; wq_dataset_destroy(tmp); }
    end = clock();
