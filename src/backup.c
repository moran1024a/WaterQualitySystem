                     ...
                     size_t path_size)
{
    ...
    if (dataset == NULL || backup_dir == NULL || backup_path_out == NULL || path_size == 0U)
        return WQ_ERROR;
    if (wq_backup_dir_ready(backup_dir) != WQ_SUCCESS)
        return WQ_ERROR;
    ...
    if (wq_write_csv(path, dataset) != WQ_SUCCESS)
        return WQ_ERROR;
...
{
    size_t count = 0U;
    if (backup_dir == NULL || backup_files == NULL || count_out == NULL)
        return WQ_ERROR;
    *count_out = 0U;
#ifdef _WIN32
    {
...
WaterQualityDataset *wq_restore_backup(const char *backup_file, DataOverview *overview)
{
    ...
    if (backup_file == NULL)
        return NULL;
    if (wq_validate_backup_file(backup_file) != WQ_SUCCESS)
        return NULL;
