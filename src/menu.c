...
{
    int v = 0;
    if (wq_read_int(prompt, &v) != WQ_SUCCESS)
        return false;
    return v == 1;
}
...
static int read_parameter(WQParameter *parameter_out)
{
    ...
    if (parameter_out == NULL)
        return WQ_ERROR;
    if (wq_read_int("参数(0 Temp,1 Salinity,2 pH,3 DO,4 precipitation,5 Air_temp): ", &p) != WQ_SUCCESS)
        return WQ_ERROR;
...
static int save_clean_dataset(SystemContext *context)
{
    if (context == NULL || context->dataset == NULL)
        return WQ_ERROR;
    if (wq_write_csv(WQ_CLEAN_CSV_FILE, context->dataset) != WQ_SUCCESS)
        return WQ_ERROR;
    if (wq_write_binary(WQ_CLEAN_BIN_FILE, context->dataset) != WQ_SUCCESS)
        return WQ_ERROR;
                                     ...
                                     double max_value)
{
    size_t i;
    size_t count = 0U;
    if (dataset == NULL || min_value > max_value)
        return 0U;
...
static void browse_pages(const WaterQualityDataset *dataset)
{
    ...
    for (;;)
    {
        ...
        if (wq_read_int("选择: ", &op) != WQ_SUCCESS)
            return;
        if (op == 0)
            return;
        if (op == 1 && page + 1U < total)
            page++;
        else if (op == 2 && page > 0U)
            page--;
        else if (op == 3)
        {
            int target;
...
static void handle_data(SystemContext *context)
{
    ...
    for (;;)
    {
        ...
        if (op == 0)
            return;
        if (ensure_data(context) != WQ_SUCCESS)
            continue;

        if (op == 1)
        {
            if (req(&context->current_user, WQ_OP_BROWSE_DATA) != WQ_SUCCESS)
                continue;
        ...
        else if (op == 2)
        {
            ...
            if (req(&context->current_user, WQ_OP_FILTER_DATA) != WQ_SUCCESS)
                continue;
            if (read_parameter(&parameter) != WQ_SUCCESS)
                continue;
            if (wq_read_double("最小值: ", &min_value) != WQ_SUCCESS ||
                wq_read_double("最大值: ", &max_value) != WQ_SUCCESS)
                continue;
        ...
        else if (op == 3)
        {
            WQParameter parameter;
            int order;
            if (req(&context->current_user, WQ_OP_SORT_DATA) != WQ_SUCCESS)
                continue;
            if (read_parameter(&parameter) != WQ_SUCCESS)
                continue;
            if (wq_read_int("排序(0升序 1降序): ", &order) != WQ_SUCCESS)
                continue;
        ...
        else if (op == 4)
        {
            ...
            WQParameter parameter;
            double value;
            if (req(&context->current_user, WQ_OP_MODIFY_DATA) != WQ_SUCCESS)
                continue;
            if (wq_read_int("记录下标: ", &idx) != WQ_SUCCESS || idx < 0)
                continue;
            if (read_parameter(&parameter) != WQ_SUCCESS)
                continue;
            if (wq_read_double("新值: ", &value) != WQ_SUCCESS)
                continue;
            if (create_auto_backup(context, "修改") != WQ_SUCCESS)
                continue;
        ...
        {
            int idx;
            if (req(&context->current_user, WQ_OP_DELETE_DATA) != WQ_SUCCESS)
                continue;
            ...
            if (!read_confirm("确认删除该记录? 1是 0否: "))
                continue;
            if (create_auto_backup(context, "删除") != WQ_SUCCESS)
                continue;
        ...
        else if (op == 6)
        {
            ...
            if (count == 0U)
                continue;
            if (!read_confirm("确认批量删除? 1是 0否: "))
                continue;
            if (create_auto_backup(context, "批量删除") != WQ_SUCCESS)
                continue;
            printf("实际删除 %lu 条记录。\n", (unsigned long)wq_delete_records_by_range(context->dataset, parameter, min_value, max_value));
        }
        else if (op == 7)
        {
            if (req(&context->current_user, WQ_OP_SAVE_DATA) != WQ_SUCCESS)
                continue;
            puts(save_clean_dataset(context) == WQ_SUCCESS ? "保存成功。" : "保存失败。");
        }
        else
...
static void handle_preprocess(SystemContext *context)
{
    ...
    for (;;)
    {
        ...
        if (op == 0)
            return;
        if (req(&context->current_user, WQ_OP_PREPROCESS_DATA) != WQ_SUCCESS)
            continue;
        if (ensure_data(context) != WQ_SUCCESS)
            continue;
        if (op == 1)
        ...
        {
            (void)wq_write_overview_report(WQ_OVERVIEW_FILE, &context->overview);
            if (wq_view_text_report(WQ_OVERVIEW_FILE) != WQ_SUCCESS)
                puts("报告不存在或无法读取。");
        }
        else
        {
...
static void handle_statistics(SystemContext *context)
{
    ...
    for (;;)
    {
        ...
        if (op == 0)
            return;
        if (req(&context->current_user, WQ_OP_STATISTICS_ANALYSIS) != WQ_SUCCESS)
            continue;
        if (ensure_data(context) != WQ_SUCCESS)
            continue;
        if (op == 1)
        ...
        {
            if (wq_view_text_report(WQ_STATISTICS_REPORT_FILE) != WQ_SUCCESS)
                puts("报告不存在或无法读取。");
        }
        else
        {
...
static void handle_prediction(SystemContext *context)
{
    ...
    for (;;)
    {
        ...
        if (op == 0)
            return;
        if (req(&context->current_user, WQ_OP_PREDICTION_ANALYSIS) != WQ_SUCCESS)
            continue;
        if (ensure_data(context) != WQ_SUCCESS)
            continue;
        if (op == 1)
        ...
        {
            if (model_count == 0U)
                (void)wq_compare_do_single_factor_models(context->dataset, models, 8U, &model_count);
...
static void handle_backup(SystemContext *context)
{
    ...
    for (;;)
    {
        ...
        if (op == 1)
        {
            if (req(&context->current_user, WQ_OP_BACKUP_DATA) != WQ_SUCCESS)
                continue;
            if (wq_create_backup(context->dataset, WQ_BACKUP_DIR, path, sizeof(path)) == WQ_SUCCESS)
            {
        ...
        else if (op == 3)
        {
            ...
            if (req(&context->current_user, WQ_OP_RESTORE_DATA) != WQ_SUCCESS)
                continue;
            ...
            if (!read_confirm("恢复会替换当前内存数据，确认恢复? 1是 0否: "))
                continue;
            if (create_auto_backup(context, "恢复") != WQ_SUCCESS)
                continue;
...
static void handle_user(SystemContext *context)
{
    ...
    for (;;)
    {
        ...
        if (op == 1)
        {
            ...
            char password[64];
            int role;
            if (req(&context->current_user, WQ_OP_CREATE_USER) != WQ_SUCCESS)
                continue;
            printf("用户名: ");
            ...
            wq_trim(username);
            wq_trim(password);
            if (wq_read_int("角色(0管理员 1普通用户 2访客): ", &role) != WQ_SUCCESS)
                continue;
        ...
        {
            if (req(&context->current_user, WQ_OP_EXPORT_USERS) != WQ_SUCCESS)
                continue;
...
void wq_handle_main_menu(SystemContext *context, int option)
{
    ...
    if (req(&context->current_user, operation) != WQ_SUCCESS)
        return;
    ...
    case 5:
        ...
        break;
    case 6:
        if (wq_view_text_report(WQ_WARNING_REPORT_FILE) != WQ_SUCCESS)
            puts("报告不存在或无法读取。");
        break;
    case 7:
        if (wq_view_text_report(WQ_STATISTICS_REPORT_FILE) != WQ_SUCCESS)
        ...
        {
            if (wq_view_text_report(WQ_PREDICTION_REPORT_FILE) != WQ_SUCCESS)
                puts("预测报告不存在或无法读取。");
        }
        break;
    case 8:
...
void wq_run_system(void)
{
    ...
    if (!context.data_loaded)
        puts("未自动加载数据，请检查数据文件。 ");

    for (;;)
    {
        wq_show_main_menu(context.current_user.role);
        if (wq_read_int("请选择操作: ", &option) != WQ_SUCCESS)
            break;
        ...
        if (option == 0)
        {
            if (wq_read_int("确认退出? 1是 0否: ", &confirm) == WQ_SUCCESS && confirm == 1)
                break;
            continue;
        }
