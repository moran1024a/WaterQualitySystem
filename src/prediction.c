                               ...
                               LinearRegressionModel *model)
{
    ...
    for (i = 0U; i < dataset->size; ++i)
    {
        ...
        double x, y;
        if (!wq_valid_xy(r, x_param, y_param))
            continue;
                             ...
                             const LinearRegressionModel *model)
{
    ...
    for (i = 0U; i < dataset->size; ++i)
    {
        if (!wq_valid_xy(&dataset->records[i], model->x_param, model->y_param))
            continue;
    ...
    for (i = 0U; i < dataset->size; ++i)
    {
        const WaterQualityRecord *r = &dataset->records[i];
        double y, yhat, d;
        if (!wq_valid_xy(r, model->x_param, model->y_param))
            continue;
                                ...
                                LinearRegressionModel *model_out)
{
    ...
    if (dataset == NULL || model_out == NULL || train_ratio <= 0.0 || train_ratio >= 1.0 || dataset->size < 10U)
        return -1.0;

    split = (size_t)((double)dataset->size * train_ratio);
    if (split < 2U || split >= dataset->size)
        return -1.0;
                                       ...
                                       size_t *model_count)
{
    ...
    size_t i;
    size_t n = 0U;
    if (dataset == NULL || models == NULL || model_count == NULL || max_models < 4U)
        return WQ_ERROR;

    for (i = 0U; i < 4U; ++i)
    {
        LinearRegressionModel m;
        if (wq_train_linear_regression(dataset, xs[i], WQ_PARAM_DO, &m) != WQ_SUCCESS)
            continue;
        m.r_squared = wq_evaluate_r_squared(dataset, &m);
        {
