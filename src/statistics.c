                                    ...
                                    ParameterStatistics *result)
{
    ...
    if (dataset == NULL || result == NULL || parameter < 0 || parameter >= WQ_PARAM_COUNT)
        return WQ_ERROR;
...
double wq_compute_mean(const WaterQualityDataset *dataset, WQParameter parameter)
{
    ...
    size_t count = 0U;
    double sum = 0.0;
    if (dataset == NULL || parameter < 0 || parameter >= WQ_PARAM_COUNT)
        return 0.0;
    for (i = 0U; i < dataset->size; ++i)
    {
        if (!wq_valid_value(&dataset->records[i], parameter))
            continue;
                              ...
                              WQParameter y_param)
{
    ...
    if (dataset == NULL || x_param < 0 || y_param < 0 || x_param >= WQ_PARAM_COUNT || y_param >= WQ_PARAM_COUNT)
        return 0.0;
    ...
    for (i = 0U; i < dataset->size; ++i)
    {
        const WaterQualityRecord *r = &dataset->records[i];
        double x, y;
        if (!wq_valid_value(r, x_param) || !wq_valid_value(r, y_param))
            continue;
                            ...
                            size_t *warning_count)
{
    ...
    if (dataset == NULL || warnings == NULL || warning_count == NULL)
        return WQ_ERROR;
                                 ...
                                 size_t *warning_count)
{
    ...
    {
        const WaterQualityRecord *r = &dataset->records[i];
        if (!wq_valid_value(r, WQ_PARAM_SALINITY))
            continue;
                                ...
                                size_t *warning_count)
{
    ...
    if (dataset == NULL || warnings == NULL || warning_count == NULL)
        return WQ_ERROR;
    if (wq_analyze_dawn_hypoxia(dataset, warnings, max_warnings, &count) != WQ_SUCCESS)
        return WQ_ERROR;
    if (wq_analyze_salinity_mutation(dataset, warnings, max_warnings, &count) != WQ_SUCCESS)
        return WQ_ERROR;
    *warning_count = count;
    return WQ_SUCCESS;
}
